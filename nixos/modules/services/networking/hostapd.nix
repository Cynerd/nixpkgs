  ...
  options_bss = {
    ...
    bridge = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        interface is included in a bridge. This parameter is not used with Host
        AP driver. If the bridge parameter is not set, the drivers will
        automatically figure out the bridge interface (assuming sysfs is enabled
        and mounted to /sys) and this parameter may not be needed.

        For nl80211, this parameter can be used to request the AP interface to
        be added to the bridge automatically (brctl may refuse to do this before
        hostapd has been started to change the interface mode). If needed, the
        bridge interface is also created.
      '';
    };
    ...
    wpaPassphrase = mkOption {
      ...
      description = lib.mdDoc ''
        WPA-PSK (pre-shared-key) passphrase. Clients will need this passphrase
        to associate with this access point.

        Warning: This passphrase will get put into a world-readable file in the
        Nix store! See {option}`environmentFile` on how to overcome that.
      '';
    };
  ...
  options_interface =
    ...
    // {
      ...
      require_ht = mkOption {
        type = types.bool;
        default = true;
        description = lib.mdDoc "Require stations to support VHT PHY (reject association if they do not)";
      };

      ieee80211ac = mkOption {
        type = types.bool;
        default = false;
        description = lib.mdDoc "Whether IEEE 802.11ac (VHT) is enabled";
      };
  ...
  in {
    ...
    script = let
      ...
      finalConfig =
        ...
        then "$RUNTIME_DIRECTORY/hostapd.conf"
        else configFile;
    in ''
      ${optionalString (configFile != finalConfig) ''
        substitueAll "${configFile}" "${finalConfig}"
      ''}
      exec hostapd "${finalConfig}"
    '';
  };
in {
    ...
    services.hostapd = {
      ...
      environmentFile = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          See section `EnvironmentFile=` in {manpage}`systemd.exec(5)` for a
          syntax reference.

          Secrets (PSKs, passwords, etc.) can be provided without adding them to
          the world-readable Nix store by defining them in the environment file
          and referring to them in option {option}`services.hostapd` with the
          syntax `@varname@`.
        '';
      };
